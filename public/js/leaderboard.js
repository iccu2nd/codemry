...
function trophyIconSvg() {
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8 21h8M12 17v4M7 4h10v5a5 5 0 0 1-10 0V4Z"/><path d="M7 5H4a2 2 0 0 0 0 4h.5M17 5h3a2 2 0 0 1 0 4h-.5"/></svg>`
}
...
function lbNameHtml(row) {
  return `<div class="lb-row-name">
    <span class="lb-nick">${escapeHtml(row.nickname || row.username)}</span>${badgesHtml(row.badges)}${devBadgeHtml(row.isDeveloper)}${roleBadgeHtml(row.role)}
  </div>`
}
...
function podiumHtml(rows, unit) {
  ...
  <div class="lb-podium">
    ${order.map(({ row, rank }) => {
      if (!row) return '<div class="lb-podium-slot empty" aria-hidden="true"></div>'
      const tier = rank === 1 ? 'gold' : rank === 2 ? 'silver' : 'bronze'
      return `
      <a class="lb-podium-slot lb-podium-${tier}" href="${profileUrl(row.username)}">
        <span class="lb-medal lb-medal-${tier}">${rank}</span>
        <div class="lb-podium-avatar">${avatarHtml(row.avatar, row.nickname || row.username, rank === 1 ? 'avatar-circle-md' : 'avatar-circle-sm')}</div>
      ...
      app.innerHTML = `
        <div class="card lb-card">
          <div class="lb-hero">
            ...
            <p class="lb-hero-sub">Top developers on Codery</p>
          </div>
          <div class="lb-tabs" role="tablist">
            ${LB_TABS.map(t => `<button type="button" role="tab" class="lb-tab-btn ${t.key === activeKey ? 'active' : ''}" data-key="${t.key}" aria-selected="${t.key === activeKey}">${t.label}</button>`).join('')}
          </div>
          ${rows.length ? `
            ${podiumHtml(top3, tab.unit)}
            ${rest.length ? `<div class="lb-divider"></div><div class="lb-list">${rest.map((row, i) => lbRowHtml(row, i + 4, tab.unit)).join('')}</div>` : ''}
