...
async function init() {
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div id="bookmarksList">${snippets.length ? snippets.map(snippetCard).join('') : `<div class="card">${emptyStateHtml({ mascot: 'noCode', title: 'No saved code yet', sub: 'Tap the bookmark icon on any code to save it here.' })}</div>`}</div>
    `
